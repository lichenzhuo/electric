<template>
  <div class="home">
    ...
    <!-- <div class="UnRead">未读消息111</div> -->
    <el-tabs v-model="activeName" @tab-click="handleClick">
      <el-tab-pane label="全部消息" name="one">
        <div class="table">
          <el-table :data="tableData" border style="width: 100%" header-cell-class-name="tablebg">
            ...
            <el-table-column align="center" label="I段母线正对地电压">
              <template slot-scope="scope">
                <span
                  :class="[{'red':scope.row.I_Vol_OffsetState!==0,}]"
                >{{scope.row.I_Position_Vol}}V</span>
              </template>
            </el-table-column>
            <el-table-column align="center" label="I段母线负对地电压">
              <template slot-scope="scope">
                <span
                  :class="[{'red':scope.row.I_Vol_OffsetState!==0,}]"
                >{{scope.row.I_Negative_Vol}}V</span>
              </template>
            </el-table-column>
            ...
            <el-table-column align="center" label="I段蓄电接地点">
              <template slot-scope="scope">
                <span
                  :class="[{'red':scope.row.I_Battery_Alarm!==0,}]"
                >{{scope.row.I_Grounding_Point}}</span>
              </template>
            </el-table-column>
            ...
            <el-table-column align="center" label="II段母线纹波">
              <template slot-scope="scope">
                <span
                  :class="[{'red':scope.row.II_Bus_RippleState!==0,}]"
                >{{scope.row.II_Bus_Ripple}}mV</span>
              </template>
            </el-table-column>
            ...
            <el-table-column align="center" label="II段蓄电池绝缘">
              <template slot-scope="scope">
                ...
                >{{scope.row.II_Battery_Ins}}K</span>
              </template>
            </el-table-column>
            ...
            <el-table-column align="center" label="母联总绝缘">
              <template slot-scope="scope">
                <span
                  :class="[{'red':scope.row.Bus_UnionStates!==0,}]"
                >{{scope.row.Busbar_Total_Ins}}K</span>
              </template>
            </el-table-column>
      ...
      <el-tab-pane label="未读信息" name="two">
        <div class="table">
          <el-table
            :data="tableDataone"
            border
...
export default {
  ...
  methods: {
    ...
    getdata() {
      ...
      // 总数据数
      this.$axios
        .post("MachineData/GetErrorAllCount", { AllRead: 1 })
        .then(res => {
      ...
      // 未读总数据数
      this.$axios
        .post("MachineData/GetErrorAllCount", { AllRead: 0 })
        .then(res => {
          console.log(res.data.Data, "这是总数");
          this.totalone = res.data.Data;
        });
    },
    changemake(e) {
      console.log(e, "父");
